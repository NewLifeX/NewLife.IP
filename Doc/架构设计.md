...
| 多格式解析 | `Parse` 自动识别 ip-api（顶层）/ 腾讯（`result.ad_info`）/ 高德（`province`/`city`）/ 百度（`content.address_detail`）响应结构 |
| 国内厂商 | `Key` 配置 + `Server` 模板 `{0}`=IP / `{1}`=Key，支持腾讯 / 高德 / 百度免费 Key 接入，中国境内省市区县高精度 |

### 6.2 核心组件

| 组件 | 所在工程 | 说明 |
|------|----------|------|
| `OnlineIp` | NewLife.IP | 实现 `IIPResolver`；`Server`/`Key`/`Timeout`/`Cache`/`Expire` 可配置，`Parse` 多格式自动识别 |
...
| 国内厂商支持 | 不支持 / 内置多格式解析 | 内置（Key + 多格式自动识别） | 淘宝 IP 库已停服（实测 301→404）；腾讯 / 高德 / 百度需免费 Key；多格式自动识别免切换配置 |
